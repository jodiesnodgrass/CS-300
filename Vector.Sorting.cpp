...
int partition(vector<Bid>& bids, int begin, int end) {
    ...
    while (!finished) { // while not done 
        ...
        if (1 >= high) {   /* If there are zero or one elements remaining,
...
void selectionSort(vector<Bid>& bids) {
    ...
    for (unsigned place = 0; place < largest; ++place) {
        ...
        for (unsigned j = place + 1; j < largest; ++j) {
            if (bids[j].title.compare(bids[smallest].title) < 0) {// if this element's title is less than minimum title
                smallest = j; // this element becomes the minimum
            }
        }
        if (smallest != place) // swap the current minimum with smaller one found
            swap(bids[place], bids[smallest]); // swap is a built in vector method
    }
}
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 3:
            ...
            selectionSort(bids);
            cout << bids.size() << " bids read" << endl;
            // calculates current clock ticks minus starting clock ticks
            ticks = clock() - ticks;
            cout << "time: " << ticks << " clock ticks" << endl;
            cout << "time: " << ticks * 1.0 / CLOCKS_PER_SEC << " seconds" << endl;
        ...
        case 4://quickSort is used to sort loaded bids and time it takes and displays
            ticks = clock();
